rm(list=ls())
load("data/example_dat.RData")
library(lmerTest)
library(effects)
library(car)

############################################################
# this script contains an hypothetical pipeline with mixed models.
############################################################
# the data are the same of the example code.
# this (fake) experiment is an hypothetic experiment with two categorical variables
# - PrimeType: negative vs positive
# - StimType: Metaphor vs literal
#
...
# - Concreteness
#
#  To give some additional context. Let's suppose that according to one theory, effect of Imageability can depend on the type and be higher for Metaphors as compared to literal stimuli.
#
#
